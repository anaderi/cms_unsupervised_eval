...
def check_sample_weight(y, sample_weight):
...
def plot_distances(X, n_neighbours=15, plot_neighs=None):
    """
    Plots the distribution of distances to the neighbour #x
    :param X:
    :param n_neighbours:
    :param plot_neighs:
    """
...
def gauss_data(n_samples=1000, classes=5):
    from sklearn.datasets import make_blobs
    return make_blobs(n_samples=n_samples, n_features=10, centers=classes)
...
def demo():
    ...
    # data, labels = sandwich_data(dist=0.2)
    ...
    for stage, transform in enumerate(generate_linear_transform(data, labels, n_neighbors=30, iterations=1000, eps=0.1,
        max_class_data_on_iteration=50, balance_forces=True)):
        print transform
        if stage % 100 == 0:
